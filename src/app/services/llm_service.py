        ...
        **kwargs
        ...
        try:
            if self.provider == "openai":
                ...
                return self._generate_mistral(prompt, system_prompt, temperature, max_tokens, **kwargs)
            else:
                raise ConfigurationException(f"Unsupported provider: {self.provider}")
                
        except Exception as e:
            raise ExternalServiceException(f"LLM generation failed: {str(e)}", context={"provider": self.provider})
