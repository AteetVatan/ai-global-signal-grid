...
class EmbeddingService:
    ...
    def _initialize_models(self):
        ...
        try:
            ...
            raise ConfigurationException(f"Embedding service initialization failed: {str(e)}")
        ...
        with measure_execution_time("embed_text"):
            try:
                ...
                if use_cache and request.cache_key in self._cache:
                    ...
                    self.logger.debug(f"Embedding cache hit for key: {request.cache_key}")
    ...
    async def _embed_with_model(self, request: EmbeddingRequest) -> EmbeddingResult:
        ...
        try:
            ...
            raise EmbeddingException(f"Embedding with model {request.model.value} failed: {str(e)}")
    ...
    async def _embed_with_sentence_transformers(self, request: EmbeddingRequest) -> EmbeddingResult:
        ...
        try:
            ...
            raise EmbeddingException(f"Sentence Transformers embedding failed: {str(e)}")
                ...
                for i, result in enumerate(results):
                    if isinstance(result, Exception):
                        ...
                        embedding_results.append(EmbeddingResult(
                            ...
                            model=model or EmbeddingModel.OPENAI_TEXT_EMBEDDING_ADA_002,
                            dimensions=1536
                        ))
                    else:
                        embedding_results.append(result)
                
                self.logger.info(f"Batch embedding completed: {len(embedding_results)} texts")
        ...
        with measure_execution_time("cluster_embeddings"):
            try:
                ...
                if method == "kmeans":
                    ...
                    cluster_labels = await loop.run_in_executor(None, kmeans.fit_predict, embedding_array)
                
                elif method == "hierarchical":
                    from sklearn.cluster import AgglomerativeClustering
                    clustering = AgglomerativeClustering(n_clusters=n_clusters)
                    cluster_labels = await loop.run_in_executor(None, clustering.fit_predict, embedding_array)
    ...
    def get_model_info(self, model: EmbeddingModel) -> Dict[str, Any]:
        ...
        model_info = {
            "name": model.value,
            "type": "openai" if model.value.startswith("text-embedding") else "sentence_transformers",
            "dimensions": None,
            "max_tokens": None,
            "loaded": False
        }
        ...
        try:
            ...
            if self._openai_client:
                try:
                    ...
                    test_result = await self.embed_text("test", model=EmbeddingModel.OPENAI_TEXT_EMBEDDING_ADA_002)
