...
class DataProcessingPipeline:
    ...
    async def process_articles(self, articles: List[Article], 
                             stages: Optional[List[ProcessingStage]] = None) -> List[ProcessedArticle]:
    ...
    async def _validate_articles(self, articles: List[Article]) -> List[Article]:
        ...
        for article in articles:
            try:
                ...
                logger.warning("Article validation failed", article_id=article.id, error=str(e))
                continue
        
        logger.info("Article validation completed", input=len(articles), output=len(valid_articles))
        ...
        for article in articles:
            try:
                ...
                if article.description:
                    ...
                    article.description = re.sub(r'[^\w\s\-.,!?]', '', article.description)
                ...
                logger.warning("Article cleaning failed", article_id=article.id, error=str(e))
                continue
        
        logger.info("Article cleaning completed", input=len(articles), output=len(cleaned_articles))
    ...
    async def _enrich_articles(self, articles: List[Article]) -> List[ProcessedArticle]:
                ...
                try:
                    ...
                    if self.nlp:
                        ...
                        processed.geographic_entities = [
                            ent for ent in processed.entities 
                    ...
                    logger.warning("Article enrichment failed", article_id=article.id, error=str(e))
                    return None
        ...
        logger.info("Article enrichment completed", input=len(articles), output=len(enriched_articles))
        return enriched_articles
    
    async def _analyze_articles(self, articles: List[ProcessedArticle]) -> List[ProcessedArticle]:
        ...
        for article in articles:
            try:
                ...
                logger.warning("Article analysis failed", article_id=article.original.id, error=str(e))
                continue
        
        logger.info("Article analysis completed", input=len(articles), output=len(analyzed_articles))
        return analyzed_articles
    
    async def _aggregate_articles(self, articles: List[ProcessedArticle]) -> List[ProcessedArticle]:
        ...
        logger.info("Article aggregation completed", input=len(articles), output=len(unique_articles))
    ...
    async def _calculate_quality_score(self, article: ProcessedArticle) -> float:
        ...
        if any(source in article.original.source.lower() for source in reliable_sources):
            score += 0.2
        
        # Recency
        hours_old = (datetime.now() - article.original.published_at).total_seconds() / 3600
        if hours_old < 24:
            score += 0.2
    ...
    async def _calculate_relevance_score(self, article: ProcessedArticle) -> float:
        ...
        # Keywords
        relevant_keywords = ["diplomacy", "foreign", "international", "policy", "relations"]
        if any(keyword in article.keywords for keyword in relevant_keywords):
            score += 0.2
        
        return min(score, 1.0)
    
    async def _deduplicate_articles(self, articles: List[ProcessedArticle]) -> List[ProcessedArticle]:
        ...
        try:
            ...
            unique_articles = [articles[i] for i in range(len(articles)) if i not in duplicates]
            
        except Exception as e:
            logger.warning("Deduplication failed, returning original articles", error=str(e))
    ...
    async def analyze_trends(self, articles: List[ProcessedArticle], 
        ...
        for topic in topics[:10]:  # Limit to top 10 topics
            try:
                trend = await self._analyze_topic_trend(topic, period_articles, start_date, end_date)
                if trend:
                    trends.append(trend)
            except Exception as e:
    ...
    async def _analyze_topic_trend(self, topic: str, period_articles: Dict, 
        ...
        for stage in ProcessingStage:
            ...
            if stage_metrics:
                total_metrics["stage_metrics"][stage.value] = {
                    ...
                    "average_time": statistics.mean([m.processing_time or 0 for m in stage_metrics])
                }
