...
class StreamFilter(BaseModel):
    ...
    def matches_article(self, article: ProcessedArticle) -> bool:
        ...
        # Category filter
        if self.categories and not any(cat in article.categories for cat in self.categories):
            return False
        
        # Country filter
        if self.countries and not any(country in article.original.country for country in self.countries):
            return False
        
        # Keyword filter
        if self.keywords and not any(keyword.lower() in ' '.join(article.keywords).lower() for keyword in self.keywords):
            return False
...
class StreamingService:
    ...
    async def connect_client(self, websocket: WebSocket, client_id: str, 
                           filters: Optional[StreamFilter] = None) -> str:
    ...
    async def _send_event_to_client(self, client_id: str, event: StreamEvent):
        ...
        try:
            ...
            logger.error("Failed to send event to client", client_id=client_id, error=str(e))
    ...
    async def _stream_data_loop(self):
        ...
        async with self.data_sources:
            async for articles in self.data_sources.stream_articles(interval_seconds=300):
                try:
                    # Process articles
                    processed_articles = await self.processing_pipeline.process_articles(articles)
                    ...
                    if len(processed_articles) > 10:
                        trends = await self.processing_pipeline.analyze_trends(processed_articles)
    ...
    async def _update_client_filters(self, client_id: str, filters_data: Dict[str, Any]):
    ...
    async def _trigger_event_handlers(self, event: StreamEvent):
        ...
        for handler in handlers:
            try:
                ...
                logger.error("Event handler failed", handler=handler.__name__, error=str(e))
