...
class DataSourcesService:
    ...
    async def _make_request(self, url: str, params: Optional[Dict] = None, headers: Optional[Dict] = None) -> str:
        ...
        try:
            ...
            for entry in feed.entries[:50]:  # Limit to 50 articles
                try:
                    ...
                    logger.warning("Failed to parse RSS entry", error=str(e), entry=entry.get('title', 'Unknown'))
                    continue
            
            self.cache[cache_key] = articles
            logger.info("Fetched Google News RSS articles", source=source_name, count=len(articles))
            return articles
            
        except Exception as e:
            logger.error("Failed to fetch Google News RSS", source=source_name, error=str(e))
            raise DataSourceError(f"Failed to fetch RSS: {e}")
    
    async def fetch_gdelt_events(self, query: str, start_date: Optional[datetime] = None, 
                                end_date: Optional[datetime] = None) -> List[GDELTEvent]:
            ...
            for item in data.get("articles", []):
                try:
                    event = GDELTEvent(
                        ...
                        event_time=datetime.strptime(item.get("seendate", "20240101"), "%Y%m%d%H%M%S"),
    ...
    async def fetch_all_sources(self, max_age_hours: int = 24) -> List[Article]:
        ...
        for source_name, source in self.sources.items():
            ...
            try:
                ...
                logger.error("Failed to fetch from source", source=source_name, error=str(e))
                continue
    ...
    async def stream_articles(self, interval_seconds: int = 300) -> AsyncGenerator[List[Article], None]:
    ...
    def add_custom_source(self, config: DataSourceConfig):
        ...
        logger.info("Added custom data source", name=config.name, type=config.source_type.value) 