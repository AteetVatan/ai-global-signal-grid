...
class DataAnalyticsService:
    ...
    async def analyze_trends(self, articles: List[ProcessedArticle], 
                           period_days: int = 7) -> List[TrendAnalysis]:
        ...
        try:
            ...
            for topic in topics[:15]:  # Top 15 topics
                trend = await self._analyze_topic_trend(topic, time_buckets, start_date, end_date)
            ...
            start_metrics.complete(len(trends), statistics.mean([t.confidence for t in trends]))
    ...
    async def analyze_geographic_patterns(self, articles: List[ProcessedArticle]) -> List[GeographicAnalysis]:
        ...
        try:
            ...
            for country_code, country_articles_list in country_articles.items():
                ...
                # Calculate metrics
                sentiments = [a.sentiment for a in country_articles_list if a.sentiment is not None]
                ...
                key_topics = [topic for topic, count in Counter(all_keywords).most_common(5)]
                
                # Calculate risk score
                risk_score = await self._calculate_geographic_risk(country_articles_list)
                
                # Determine trend direction
                trend_direction = await self._determine_trend_direction(country_articles_list)
            ...
            logger.info("Geographic analysis completed", countries_count=len(geographic_analyses))
    ...
    async def analyze_correlations(self, articles: List[ProcessedArticle]) -> List[CorrelationAnalysis]:
        ...
        try:
            ...
            for var1, var2 in correlation_pairs:
                if var1 in df.columns and var2 in df.columns:
                    ...
                    interpretation = await self._interpret_correlation(var1, var2, correlation, p_value)
            ...
            logger.info("Correlation analysis completed", correlations_count=len(correlations))
            return correlations
    ...
    async def assess_risks(self, articles: List[ProcessedArticle]) -> List[RiskAssessment]:
        ...
        try:
            ...
            for risk_type, assessor_func in risk_types:
                try:
                    ...
                    logger.warning(f"Risk assessment failed for {risk_type}", error=str(e))
                    continue
    ...
    async def generate_comprehensive_report(self, articles: List[ProcessedArticle], 
        ...
        try:
            ...
            # Generate summary
            summary = await self._generate_summary(articles, trends, geographic_analysis, risk_assessments)
            
            # Generate visualizations
            visualizations = await self._generate_visualizations(articles, trends, geographic_analysis)
    ...
    async def _analyze_topic_trend(self, topic: str, time_buckets: Dict, 
                                 start_date: datetime, end_date: datetime) -> Optional[TrendAnalysis]:
    ...
    async def _find_related_topics(self, topic: str, articles: List[ProcessedArticle]) -> List[str]:
        ...
        return [keyword for keyword, count in keyword_counts.most_common(5) if keyword != topic]
    
    async def _calculate_geographic_risk(self, articles: List[ProcessedArticle]) -> float:
        """Calculate risk score for geographic region."""
        if not articles:
            return 0.0
        
        # Factors for risk calculation
        negative_sentiment_ratio = len([a for a in articles if a.sentiment and a.sentiment < -0.2]) / len(articles)
        high_relevance_ratio = len([a for a in articles if a.relevance_score > 0.8]) / len(articles)
        conflict_keywords = ['conflict', 'war', 'tension', 'crisis', 'dispute']
        conflict_ratio = len([a for a in articles if any(keyword in ' '.join(a.keywords).lower() for keyword in conflict_keywords)]) / len(articles)
        
        # Weighted risk score
        risk_score = (negative_sentiment_ratio * 0.4 + 
    ...
    async def _determine_trend_direction(self, articles: List[ProcessedArticle]) -> str:
        ...
        # Calculate sentiment trend
        early_sentiments = [a.sentiment for a in sorted_articles[:len(sorted_articles)//2] if a.sentiment is not None]
        late_sentiments = [a.sentiment for a in sorted_articles[len(sorted_articles)//2:] if a.sentiment is not None]
    ...
    async def _interpret_correlation(self, var1: str, var2: str, correlation: float, p_value: float) -> str:
    ...
    async def _assess_sentiment_risk(self, articles: List[ProcessedArticle]) -> Optional[RiskAssessment]:
    ...
    async def _assess_geographic_risk(self, articles: List[ProcessedArticle]) -> Optional[RiskAssessment]:
        ...
        if high_risk_countries:
            max_risk_country, max_risk_score = max(high_risk_countries, key=lambda x: x[1])
    ...
    async def _assess_temporal_risk(self, articles: List[ProcessedArticle]) -> Optional[RiskAssessment]:
        ...
        recent_articles = [a for a in articles if a.original.published_at >= recent_cutoff]
        
        if len(recent_articles) > len(articles) * 0.5:  # High recent activity
            recent_sentiments = [a.sentiment for a in recent_articles if a.sentiment is not None]
    ...
    async def _assess_content_risk(self, articles: List[ProcessedArticle]) -> Optional[RiskAssessment]:
        ...
        for article in high_relevance_articles:
            if any(keyword in ' '.join(article.keywords).lower() for keyword in high_risk_keywords):
    ...
    async def _generate_summary(self, articles: List[ProcessedArticle], trends: List[TrendAnalysis],
                              geographic_analysis: List[GeographicAnalysis], 
        ...
        avg_sentiment = statistics.mean([a.sentiment for a in articles if a.sentiment is not None])
        ...
        # Risk summary
        high_risks = [r for r in risk_assessments if r.risk_level in ["high", "critical"]]
    ...
    async def _generate_visualizations(self, articles: List[ProcessedArticle], trends: List[TrendAnalysis],
                                     geographic_analysis: List[GeographicAnalysis]) -> Dict[str, Any]:
        ...
        try:
            ...
            if sentiments:
                ...
                fig_sentiment.update_layout(title="Sentiment Distribution", xaxis_title="Sentiment", yaxis_title="Count")
            ...
            if trends:
                ...
                fig_trends.update_layout(title="Top Trends by Confidence", xaxis_title="Topic", yaxis_title="Confidence")
    ...
    def get_analytics_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_processing_time": statistics.mean([m.processing_time or 0 for m in self.metrics]),
            "total_articles_processed": sum(m.input_count for m in self.metrics),
            "analysis_types": {
                analysis_type.value: len([m for m in self.metrics if m.analysis_type == analysis_type])
