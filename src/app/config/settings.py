...
class Settings(BaseSettings):
    ...
    flashpoint_max_iterations: int = Field(
        default=10, description="Flashpoint max iterations"
    )
    target_flashpoints: int = Field(
        default=20, description="Flashpoint target flashpoints"
    )
    ...
    mistral_token_ref: str = Field(
        default="mistral-small", description="Mistral token ref"
    )
    ...
    GDELT_API_URL_1: Optional[str] = Field(
        default=None, description="MASX AI GDELT API URL - 1"
    )
