...
async def translation_health():
    ...
    try:
        async with TranslationService() as translator:
            ...
            logger.info(f"Translation health check completed: {health_status['status']}")
            return health_status
    except Exception as e:
        logger.error(f"Translation health check failed: {e}")
        raise HTTPException(status_code=503, detail=f"Translation health check failed: {str(e)}")
...
async def embedding_health():
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"Embedding health check failed: {str(e)}")
...
async def workflows_health():
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"Workflows health check failed: {str(e)}")
