...
class LanguageResolver(BaseAgent):
    ...
    async def detect_languages(
        ...
        try:
            ...
            result = {
                ...
                "languages_detected": list(set(r["detected_language"] for r in results)),
                "items_needing_translation": len([r for r in results if r["needs_translation"]]),
    ...
    async def resolve_language_codes(
        ...
        """
        try:
            self.logger.info(f"Resolving language codes for {len(language_results)} items")
