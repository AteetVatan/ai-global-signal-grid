...
class MemoryManager(BaseAgent):
        ...
        try:
            ...
            for hotspot in hotspots:
                try:
                    ...
                    hotspot_text = (
                        f"{hotspot.get('title', '')} {hotspot.get('summary', '')}"
                    )
                    ...
                    self.logger.error(
                        f"Failed to store hotspot {hotspot.get('id', 'unknown')}: {str(e)}"
                    )
                    failed_hotspots.append({"hotspot": hotspot, "error": str(e)})
        ...
        try:
            ...
            for article in articles:
                try:
                    ...
                    self.logger.error(
                        f"Failed to store article {article.get('url', 'unknown')}: {str(e)}"
                    )
                    failed_articles.append({"article": article, "error": str(e)})
    ...
    def _extract_themes(self, hotspots: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [
                {"theme": theme, "count": count} for theme, count in sorted_themes[:10]
            ]
    ...
    def cleanup_old_data(self, days_to_keep: int = 90) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Cleanup completed: {deleted_hotspots} hotspots, "
