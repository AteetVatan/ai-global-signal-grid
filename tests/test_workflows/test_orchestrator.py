...
class TestMASXOrchestrator:
    ...
    def test_run_data_fetchers_parallel_execution(self):
        ...
        with patch('asyncio.run') as mock_asyncio_run:
            mock_asyncio_run.return_value = [mock_news_agent.state, mock_event_agent.state]
    ...
    def test_run_agent_async(self):
        ...
        async def test_async():
            result = await self.orchestrator._run_agent_async("test_agent", {"input": "test"})
            return result
    ...
    def test_run_agent_async_agent_not_available(self):
        ...
        async def test_async():
            with pytest.raises(WorkflowException, match="Agent test_agent not available"):
                await self.orchestrator._run_agent_async("test_agent", {"input": "test"})
