...
class TestDataSourcesService:
    ...
    def test_service_initialization(self, service):
        ...
        google_news_sources = [
            name
            for name, config in service.sources.items()
    ...
    async def test_fetch_gdelt_events_success(self, service):
        ...
        # Mock the HTTP request
        with patch.object(
            service, "_make_request", return_value=json.dumps(mock_gdelt_response)
        ):
            with patch.object(service, "_check_rate_limit", return_value=True):
    ...
    async def test_fetch_all_sources_with_old_articles(self, service):
        ...
        with patch.object(
            service, "fetch_google_news_rss", return_value=[old_article, new_article]
        ):
            with patch.object(service, "_check_rate_limit", return_value=True):
